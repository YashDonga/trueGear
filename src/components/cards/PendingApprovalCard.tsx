...
export function PendingApprovalCard({ 
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white rounded-[10px] border border-[#e5e7eb] p-5 transition-all cursor-pointer hover:shadow-md ${isActive ? 'ring-2 ring-[#ff4f31]' : ''}`}
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          ...
          {/* Status Badge */}
          <div className="bg-[#ffe1b7] px-4 py-1.5 rounded-[5px] shadow-[2px_4px_8px_0px_rgba(0,0,0,0.15)]">
            <p className="text-[16px] font-semibold text-[#e89d00]">{status}</p>
          </div>
        </div>
      </div>
    </div>
